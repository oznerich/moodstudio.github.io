...
async function loadAnalytics() {
  ...
  const packages = data.reduce((acc, curr) => {
    acc[curr.package] = (acc[curr.package] || 0) + 1;
    return acc;
  }, {});

  if (packages.euphoria > packages.serendipity && packages.euphoria > packages.keepsake) {
    insights.push('Euphoria is the most popular package.');
  } else {
    insights.push('Users are trying other packages more.');
  }
...
async function loadGallery() {
  const { data } = await supabase.storage.from('user-uploads').list('appointments');
  ...
  container.innerHTML = data.map(file => {
    ...
    return `<img src="${pub.publicUrl}" style="width:100px;" />`;
  }).join('');
}
